...
Empirica.onGameStart((game) => {
  ...
  players.forEach((player, i) => {
    ...
    player.set("bonus", 0);
    player.set("moveInforScore", 0); //moveInforScore -->>YWH
    player.set("newBonus", 0); //newBonus       -->>YWH
    player.set("moveNum", 0); //移动次数         -->>YWH
    player.set("wordNum", 0); //字数             -->>YWH
    player.set("finalScore", 0); //最终表现等级      -->>YWH
    player.set("sumFinalScore", 0); //最终表现等级      -->>YWH
    player.set("satisfiedTime",[]); //记录满意时间段和决策时间
  });
});
...
Empirica.onStageStart((game, round, stage) => {
  ...
  stage.set("score", 0);
  stage.set("moveNum", 0);
  stage.set("submitStatu", "timeOut");     //设置回合提交状态：超时，超次数，满意
  ...
  task.students.forEach((student) => {
    ...
    stage.set(`student-${student}-move`, null); //-->>YWH
  });
...
Empirica.onStageEnd((game, round, stage) => {
  ...
  //计算player移动学生的次数和操作分数   -->>YWH
  players.forEach((player) => {
    player.set("moveNum", getMoveTimes(player, stage));
    player.set("moveInforScore", getMoveScore(player, stage));
    player.set("wordNum", getWordNum(player, stage));
    // summoveInforScore += player.get("moveInforScore");
  });

  //将stage的chat存入到round中
  const stageChat = stage.get("chat");
  stageChat.forEach((content) => {
    // if (content.playerId === player._id) {
    //   wordNum += content.text.length;
    // }
    round.append("chat",{
      text: content.text,
...
//获得移动学生次数
function getMoveTimes(player, stage) {
  let moveNum = player.get("moveNum");
  const intermediateSolutions = stage.get("intermediateSolutions");
  intermediateSolutions.forEach((ob) => {
    if (ob.playerID === player._id) {
      moveNum += 1;
    }
  });
  return moveNum;
...
//获得操作提升分数
function getMoveScore(player, stage) {
  let moveScore = player.get("moveInforScore");
  let firstScore = 0;
  let h = 0;
  const intermediateSolutions = stage.get("intermediateSolutions");
  intermediateSolutions.forEach((ob) => {
    if (ob.completeSolution) {
      h += 1;
      if (h > 1) {
        if (ob.playerID === player._id) {
          moveScore += ob.completeSolutionScore - firstScore;
        }
      }
      firstScore = ob.completeSolutionScore;
    }
  });
  return moveScore;
}
...
//计算聊天信息量
function getWordNum(player, stage) {
  let wordNum = player.get("wordNum");
  const stageChat = stage.get("chat");
  stageChat.forEach((content) => {
    if (content.playerId === player._id) {
      wordNum += content.text.length;
...
Empirica.onGameEnd((game) => {
  ...
  players.forEach((player) => {
    ...
    player.set("finalScore", getFinalScore(player)); //-->>YWH
    // summoveInforScore += player.get("moveInforScore");
  });
  ...
  let sumFinalScore = 0;
  players.forEach((player) => {
    sumFinalScore += getFinalScore(player);
    player.set("sumFinalScore", sumFinalScore);
    // sumFinalScore += player.get("finalScore");
  });
  players.forEach((player) => {
    const finalScore = getFinalScore(player);
    const newBonus = getNewBonus(sumFinalScore, finalScore);
    player.set("newBonus", newBonus); //存在冗余计算-->>YWH
    // console.log(sumFinalScore);
  });
  //-->>YWH
  // players.forEach((player) => {
  //   const newBonus = (player.get(moveInforScore) * bonus) / summoveInforScore;
  //   if (player.get("newBonus") === 0) {
  //     //if we never computed their bonus
  //     player.set("newBonus", newBonus);
  //   }
  // });
});
...
function getFinalScore(player) {
  let finalScore = 0;
  finalScore = (
    0.5 * player.get("moveInforScore") +
    10 * player.get("moveNum") +
    1 * player.get("wordNum")
  ).toFixed(0);
  return parseInt(finalScore);
}
//计算分配金额
function getNewBonus(sumFinalScore, finalScore) {
  let basicBonus = 8; //基础金额
  let teamDivide = 6; //团队分配金额
  const bonus = (
    basicBonus +
    (finalScore * teamDivide) / sumFinalScore
  ).toFixed(1);
  return bonus;
}
...
Empirica.onSet(
  (
    ...
    if (key === "satisfied") {
      ...
      if (allSatisfied) {
        players.forEach((player) => {
          stage.set("submitStatu", "allSatisfied");
          player.stage.submit();
        });
      }
      return;
    ...
    let remainMoveNum = stage.get("task").moveTimes;
    remainMoveNum = remainMoveNum - stage.get("moveNum");
    if (remainMoveNum === 0) {
      players.forEach((player) => {
        stage.set("submitStatu", "moveNumOut");
        player.stage.submit();
      });
    }
    ...
    if (key.substring(0, 8) === "student-" && key.slice(-4) === "room") {
      ...
      // player.set(
      //   "moveInforScore",
      //   getIncrementScore(stage, player, currentScore)
      // ); //moveInforScore -->>YWH
      // player.set("moveNum", player.get("moveNum") + 1); //移动次数        -->>YWH
      ...
      const intermediateSolutions = stage.get("intermediateSolutions");
      intermediateSolutions.forEach(() => {
        currentMoveNum += 1;
      });
      stage.set("moveNum", currentMoveNum || 0);
      //记录每轮player的内容
      // let i = 0;
      ...
      //     playerID: player._id,
      //     moveNum: ,
      //     moveInforScore: getMoveScore(player, stage),
      //   });
      // });
    }
  }
...
//获取当前分数与上一次操作分数的差值   -->>YWH
function getIncrementScore(stage, player, currentScore) {
  let score = 0;
  score = player.get("moveInforScore") + currentScore - stage.get("score");
  return score;
}
